...
def get_url(url, url_type, request_num, working_memory, accept_header, http_headers):
  """
...
'''.format(req_path, urlo.netloc,
  ...
  util.printlog(log, working_memory, url_type, request_num, url, start, end, response_time, total_length)
  return 
...
def post_url(url, url_type, request_num, working_memory):
  """
  # Desc  : Function to send post requests to the server. Type 2 is post requests
...
def delete_url(url, url_type, request_num, working_memory):
  """
...
def main_entry(url, request_num, url_type, log_dir, interval,
  ...
  if url_type == 1:
    get_url(url, url_type, request_num, working_memory, accept_header, http_headers)
  if url_type == 2:
    post_url(url, url_type, request_num, working_memory)
  if url_type == 3:
    delete_url(url, url_type, request_num, working_memory)
